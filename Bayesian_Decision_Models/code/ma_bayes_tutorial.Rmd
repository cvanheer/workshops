...
run.sim <- function(noise.conditions, change.points, time.scale){
  
  ## Create a matrix containing the conditions you want to simulate data over 
  simulation <- as_tibble(expand.grid(
    tchange = change.points, ## the trial on which there is a change point 
    sigma = noise.conditions ## noise parameter
  ))
  
  ## Run the simulation for each row of these parameters in simulation
  ## Add a new column called "data" which contains the simulated data nested in a row 
  simulation <- simulation %>% 
    mutate(
      data = map(tchange, sim.data, sigma, time.scale)
    ) 
...
## Create parameter matrix to run simulations over 
# simtest <- tibble(expand.grid(true.change.point = change.points, ## the trial on which there is a change point 
#                               sigma = noise.conditions)) ## noise parameter

simtest <- purrr::map_dfr(1, function(x){
  simtest <- tibble(expand.grid(true.change.point = change.points, ## the trial on which there is a change point 
                                sigma = noise.conditions)) ## noise parameter
  return(simtest)
})
simtest$simNo <- 1:dim(simtest)[1]

## Run simulations 
simtest <- simtest %>% 
  rowwise() %>% ## use rows as input into function 
  mutate(
    data = map(true.change.point, sim.data, sigma, 10)
  ) %>% 
...
sim.summary <- simtest %>% 
  ...
  summarise(
    sigma = unique(sigma),
    true.change.point = unique(true.change.point),
    MAP = max(post.prob), ## maximum value of posterior (MAP estimate)
    pred.change.point = which(post.prob == MAP)-1
  ) 

sim.summary$correct <- NA
for (ii in 1:dim(sim.summary)[1]){
  if (sim.summary$pred.change.point[ii] - sim.summary$true.change.point[ii] == 0){
    sim.summary$correct[[ii]] <- 1
  }else{
    sim.summary$correct[[ii]] <- 0
  }
}

## Calculate prop correct
sim.prop.correct <- sim.summary %>% 
  group_by(sigma, true.change.point) %>% 
  summarise(
    mean.MAP = mean(MAP),
    prop.correct = sum(correct)/n()
  )


## Plot proportion correct
sim.prop.correct %>% 
  ggplot(., aes(x = true.change.point, y = prop.correct, group = sigma)) + 
  geom_line(aes(colour = as.factor(sigma))) + 
  xlab("True Change Point Occurred") + ylab("Proportion Correct") + 
  scale_colour_manual(
    name = "Noise Level",
